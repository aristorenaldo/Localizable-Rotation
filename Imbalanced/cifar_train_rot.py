...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = models.__dict__[args.arch](
        num_classes=num_classes, use_norm=use_norm, num_trans=4
    )
...
def train(train_loader, model, criterion, optimizer, epoch, args, log, tf_writer, CE):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        rot_label_cat = torch.stack(
            (rot0_label, rot90_label, rot180_label, rot270_label), dim=0
        )
        ...
        rotlabel = (
            rot_label_cat[torch.arange(rot_label_cat.size(0)), idx]
            .reshape(-1)
            .long()
            .cuda()
        )
        rotclass_label = (
            rotclass_label[torch.arange(rotclass_label.size(0)), idx]
            .reshape(-1)
            .long()
            .cuda()
        )
